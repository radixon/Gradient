...
"""
    ...
    Parameters:
        x:  required to make sure the constant function is part of the computation graph
...
"""
    Identity Function: 
        ...
        Parameters:
            x:  The value for which f'(x) will be evaluated 
            c:  A constant value that is defaulted to zero
"""
...
func = 4*x**6 - 3*x**5 - 10*x**2 + 5*x + 16
y = difference_rule(func)
y.backward()
print(f'f(x) = 4x**6 - 3x**5 - 10x**2 + 5x + 16')
print(f'f\'(2.0) = {x.grad} \n')

x = torch.tensor(3.0, requires_grad=True)
func = x**3 + 6*x**2 + 8*x + 1
y = difference_rule(func)
y.backward()
print(f'f(x) = x**3 - 6x**2 - 8x - 1')
